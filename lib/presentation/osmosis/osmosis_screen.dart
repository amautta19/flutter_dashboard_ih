...
class _OsmosisScreenState extends State<OsmosisScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_){
      context.read<FilterElementProvider>().updateColumn('MF01_pc');
    });
  }
  @override
                ...
                builder: (context, snapshot){
                  ...
                  return Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Expanded(child: TablaOsmosis(osmosisData: filteredData))
                        ],
                      ),
                              ...
                              Consumer<FilterDayProvider>(
                                builder: (context, dayProvider, child){
                                  return Column(
                                    children: [
                                      StreamBuilder(
                                        stream: SupabaseServices().getDataByDayOperative('osmosis', dayProvider.getDate), 
                                        builder: (context, snapshot){
                                          if (snapshot.connectionState == ConnectionState.waiting) {
                                            return SizedBox(
